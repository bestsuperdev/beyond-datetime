		...
		return (
			<div className='app'>
				...
				<div>
					<p>英文</p>
					<div>
						<Trigger wrapStyle={{ display: 'block' }} target={<DateRange language="en" {...range} time ranges={defaultRanges} onConfirm={this.handlerRangeChange.bind(this)} />}>
							<input style={{ width: '100%' }} type="text" value={range && range.defaultStartDate ? this.rangeToString(range, true) : ''} />
						</Trigger>
						<Trigger target={<Time language="en" defaultDate={date1} onConfirm={this.handlerChange.bind(this, 'date1')} />}>
							<input type="text" value={date1 ? date1.toLocaleTimeString() : ''} />
						</Trigger>

						<Trigger target={<Calendar language="en" invalidDates={isInValidDate} defaultDate={date2} onChange={this.handlerChange.bind(this, 'date2')} />}>
							<input type="text" value={date2 ? date2.toLocaleDateString() : ''} />
						</Trigger>
						<Trigger target={<Calendar language="en" invalidDates={isInValidDate} defaultDate={date2} time onConfirm={this.handlerChange.bind(this, 'date2')} />}>
							<input type="text" value={date2 ? date2.toLocaleString() : ''} />
						</Trigger>
						<Trigger target={<Calendar language="en" defaultDate={date2} time onConfirm={this.handlerChange.bind(this, 'date2')} />}>
							<input type="text" value={date2 ? date2.toLocaleString() : ''} />
						</Trigger>

						<Trigger target={<Calendar language="en" defaultDate={date2} onConfirm={this.handlerChange.bind(this, 'date2')} />}>
							<input type="text" value={date2 ? date2.toLocaleDateString() : ''} />
						</Trigger>


						<Trigger target={<DateRange language="en" {...range} onConfirm={this.handlerRangeChange.bind(this)} />}>
							<input style={{ width: 150 }} type="text" value={range ? this.rangeToString(range) : ''} />
						</Trigger>


						<Trigger type="div" className="trigger" target={<DateRange language="en" {...range} ranges={defaultRanges} onConfirm={this.handlerRangeChange.bind(this)} />}>
							<input style={{ width: 150 }} type="text" value={range ? this.rangeToString(range) : ''} />
						</Trigger>
