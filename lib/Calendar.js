...
var Calendar = function (_Component) {
	...
	function Calendar(props, context) {
		...
		date = (0, _parseInput2['default'])(date, format, true);
		_this.state = {
			date: date,
	...
	_createClass(Calendar, [{
		...
		value: function componentWillReceiveProps(nextProps) {
			var date = nextProps.date,
			    ...
			    timeDate = nextProps.timeDate;

			var nextState = null;

			if (date != null && (typeof isInvalid !== 'function' || !isInvalid(date))) {
				nextState = { date: (0, _parseInput2['default'])(date, format, true), timeDate: time ? (0, _parseInput.parseTimeInput)(date, format, true) : null };
			} else if (range && timeDate) {
				nextState = { timeDate: timeDate };
			}
		...
		value: function handlerTimeSelect(newDate, type) {
			...
			if (date == null) {
				...
				if (typeof onTimeChange === 'function') {
					_result = onTimeChange(newDate);
				}
				if (typeof onChange === 'function') {
					result2 = onChange(_date.clone(), type);
				}

				if (!link && _result !== false && result2 !== false) {
		...
		value: function renderDays() {
			...
			var _props7 = this.props,
			    ...
			    isInvalid = _props7.isInvalid;


			var shownDate = this.getShownDate();
			var _state3 = this.state,
			    date = _state3.date,
			    firstDayOfWeek = _state3.firstDayOfWeek;
		...
		value: function renderTime() {
			var _props8 = this.props,
			    time = _props8.time,
			    hour = _props8.hour,
			    minute = _props8.minute,
			    second = _props8.second,
			    confirm = _props8.confirm;
